#include "stm32f1xx_hal.h"  // 如果没include，加这个（假设F1系列，根据你的MCU改）
#include "OLED.h"
#include <string.h>
#include <math.h>
#include <stdio.h>
...
extern I2C_HandleTypeDef hi2c1;  // CubeMX生成的外部声明
static I2C_HandleTypeDef *g_pHI2COLED = &hi2c1;
#define OELD_I2C_ADDR 0x78  // 从机地址
#define OLED_TIMEOUT 500    // 超时ms
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	// uint8_t *buf = (uint8_t *)malloc(Count + 1);   // 动态分配缓冲，加控制字节
    // if (buf == NULL) return;  // 防内存炸
    // buf[0] = 0x40;  // 控制字节，数据模式
    // memcpy(&buf[1], Data, Count);
    // HAL_I2C_Master_Transmit(g_pHI2COLED, OELD_I2C_ADDR, buf, Count + 1, OLED_TIMEOUT);
    // free(buf);  // 释放内存
