...
// 【第一步：只改这一个函数，其他代码完全不动】
int16_t DS18B20_Get_Temp(void)      // 原来是 short，改成 int16_t 防止<<8溢出
{
    uint8_t TL, TH;
    int16_t raw;

    DS18B20_Start();        // 你已经在这里面延时等转换完了，完美
    ...
    raw = ((int16_t)TH << 8) | TL;       // 合成有符号原始数据

    // 下面这三行完全替换你原来的大段代码，最简最快
    if (raw < 0) { 
        raw = -raw;                      // 负数取绝对值（DS18B20负温是补码）
    }
    raw = (raw >> 4) * 10 + ((raw & 0x0F) * 10 + 8) / 16;   // 整数精确转0.1℃

    return raw;     // 25.6℃ → 返回 256，-10.5℃ → 返回 -105
}

// 【第二步：加上这一个打印函数，任务里只调用这一个函数】
void DS18B20_Print_Temp(void)
{
    int16_t t = DS18B20_Get_Temp();     // 栈只用了 2 字节！！
