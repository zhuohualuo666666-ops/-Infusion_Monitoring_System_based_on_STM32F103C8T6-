...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuespeed != cur->valuespeed)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuespeed Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagspeed = 1;
            ret = 1;
        }
    }
    if(last->valuewater != cur->valuewater)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuewater Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagwater = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagspeed)
    {
        devStatusTmp.valuespeed = gizY2X(speed_RATIO,  speed_ADDITION, dataPoints->valuespeed); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuespeed,speed_LEN);
        byteFieldOffset += speed_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwater)
    {
        devStatusTmp.valuewater = gizY2X(water_RATIO,  water_ADDITION, dataPoints->valuewater); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewater,water_LEN);
        byteFieldOffset += water_LEN;
    }
