...
{   
    ...
    while (GPIO_ReadInputDataBit(DS18B20_GPIO_PORT,DS18B20_GPIO_PIN)&&retry<200)//等待应答的过程中电平为高，延时增加，检测到电平被拉低后或延时超过200退出
	{
		retry++;
		Delay_us(1);
	};	 
    ...
    while (!GPIO_ReadInputDataBit(DS18B20_GPIO_PORT,DS18B20_GPIO_PIN)&&retry<240)//拉低DQ为低电平，应答脉冲发出后，应该变回高电平，如果240us后没有拉高，代表应答脉冲不完整，仍然返回1，表示未检测到DS18B20存在
	{
		retry++;
		Delay_us(1);
	};
...
uint8_t DS18B20_Read_Bit(void) 	 //拉低至少1us最多15us释放总线，采集总线状态
{
	...
	Delay_us(12);       //延时12us
	if(GPIO_ReadInputDataBit(DS18B20_GPIO_PORT,DS18B20_GPIO_PIN))data=1;//读取DS18B20发给单片机的信号是高电平还是低电平
    else data=0;	    
    Delay_us(50);       //延时50us    
    return data;
}
